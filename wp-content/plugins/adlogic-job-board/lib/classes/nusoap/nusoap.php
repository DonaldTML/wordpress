...
class nusoap_base {
    ...
    * @access	public
	*/
	function serialize_val($val,$name=false,$type=false,$name_ns=false,$type_ns=false,$attributes=false,$use='encoded',$soapval=false, $ns_id = null) {
...
class soapval extends nusoap_base {
	...
	* @access   public
	*/
  	function __construct($name='soapval',$type=false,$value=-1,$element_ns=false,$type_ns=false,$attributes=false, $ns_id = null) {
		parent::__construct();
		$this->name = $name;
		$this->type = $type;
		$this->value = $value;
	...
	*/
	function serialize($use='encoded') {
		return $this->serialize_val($this->value, $this->name, $this->type, $this->element_ns, $this->type_ns, $this->attributes, $use, true, $this->ns_id);
    }
...
//	    	$this->debug("in serializeType: soapval overrides value to $value");
		...
		if (strpos($type, ':')) {
			...
			} else if ($ns == 'http://example.org') {
				$this->debug('in serializeType: appears to be an example type');
				if ($uqType == 'ArrayOfArrayOfint_literal' || $uqType == 'ArrayOfint_literal') {
					$this->debug('in serializeType: Add namespace for example SOAP type');
					// Add prefix
					$tt_prefix = 'ns' . rand(1000, 9999);
					$this->namespaces[$tt_prefix] = 'http://example.org';
					// force this to be added to usedNamespaces
					$tt_prefix = $this->getPrefixFromNamespace('http://example.org');
					...
					foreach ($value as $k => $v) {
						...
						if (is_array($v)) {
							foreach ($v as $k2 => $v2) {
								$contents .= $this->serialize_val((int) $v2,'int',false,false,false,false,$use);
							}
						} else {
							$contents .= $this->serialize_val((int) $v,'int',false,false,false,false,$use);
						}
						$contents .= '</int>';
					}
					$xml = "<$name$elementNS xsi:type=\"" . $this->getPrefixFromNamespace($ns) . ":$uqType\">$contents</$name>";
					return $xml;
				}
			
				$this->debug('in serializeType: example type, but only support ArrayOfArrayOfint_literal');
			}
		} else {
			// TODO: should the type be compared to types in XSD, and the namespace
			// set to XSD if the type matches?
		...
		if ($phpType == 'struct') {
			if (isset($typeDef['typeClass']) && $typeDef['typeClass'] == 'element') {
				...
				if ($unqualified) {
					$elementNS = " xmlns=\"\"";
							...
							($typeDef['name'] == 'ApplicantSubscriberWS') || 
							($typeDef['name'] == 'ApplicantWS') || 
							($typeDef['name'] == 'ApplicantNoJobAdWS')
						) {
					$elementNS = " xmlns=\"java:au.com.martianlogic.martianjobs.dto.applicant\"";
				} else {
					$elementNS = '';
				}
			}
			...
			if (isset($typeDef['form']) && ($typeDef['form'] == 'qualified')) {
				...
				if ($unqualified) {
					$elementNS = " xmlns=\"\"";
				} else if ($typeDef['name'] == 'ApplicantSubscriberWS'){
					$elementNS = " xmlns=\"java:au.com.martianlogic.martianjobs.dto.applicant\"";
				} else {
					$elementNS = '';
				}
